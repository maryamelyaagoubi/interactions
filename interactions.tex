...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\textcolor{MidGray}{Projet Interactions}}
\fancyhead[R]{\includegraphics[height=0.7cm]{logo_mva.png}}
...
% Title page data — not used with \maketitle, kept for hyperref metadata only
\title{Ammonia Cross-Hedging}
\author{Ruben Cardoso, Maryam El Yaagoubi}
\date{\today}
...
% Top logo
\includegraphics[height=2.2cm]{logo_mva.png}\par
\vspace{1.8cm}

% Title block
{\fontsize{26}{32}\selectfont\bfseries\color{Ink} Ammonia Cross-Hedging}\par
\vspace{0.4cm}
{\large\color{MidGray} Interaction diagnostics for commodity proxy selection}\par
...
{\large
\begin{tabular}{cc}
  \textbf{Ruben Cardoso} & \textbf{Maryam El Yaagoubi} \\[4pt]
  \small\texttt{ruben.cardoso@dauphine.eu} & \small\texttt{maryam.el-yaagoubi@enpc.fr}
\end{tabular}
}\par
...
% Course label
{\color{MidGray}\itshape Projet Interactions : M2 MVA }\par
\vspace{0.6cm}
{\color{MidGray} 1\textsuperscript{er} mai 2026}\par
...
The statistical question is therefore:

\begin{center}
\textbf{``Which observable markets interact with ammonia, 
at which horizon, in which states, and in which tail?''}
\end{center}
...
Ammonia prices are physical benchmark prices. They contain logistics, contract structure, regional basis and marginal-source effects that are not fully observed in financial time series.
