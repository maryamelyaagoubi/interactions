...
\[
\text{``Is ammonia correlated with gas?''}
\]
It is sharper:
\[
\text{``Which observable markets interact with ammonia, at which horizon, in which states, and in which tail?''}
\]
...
A_t
\]
be the ammonia price at date \(t\). Let
\[
...
\]
If \(I_j>1\), proxy spikes and ammonia spikes co-occur more often than independence would suggest. The count of joint spike observations must always be reported, because tail objects are sample-sensitive.
...
A standard dynamic variance hedge would choose
\[
h_t^\star
=
\arg\min_{h}
...
\right].
\]
In the rolling covariance case, this gives
\[
h_t
=
\widehat\Sigma_{ZZ,t}^{-1}
\widehat\Sigma_{ZY,t}.
\]
This object is useful, but it should not be used blindly. The interaction analysis says that the hedge should be conditioned on at least three facts:
\[
\text{timing},
\qquad
\text{transmission strength},
\qquad
\text{stress interaction}.
\]

A more interaction-aware rule would keep the covariance hedge as a direction, but modulate its intensity using the interaction diagnostics:
\[
h_t^{\mathrm{final}}
=
\omega_t\,g_t\,\widetilde h_t .
\]
Here \(\widetilde h_t\) is a smoothed hedge direction, \(g_t\) is an activation term based on current transmission strength, and \(\omega_t\) is a stress or regime intensity. The exact implementation is secondary to the conceptual point: the hedge should be active when the proxy is currently connected to ammonia, not merely because the proxy is economically plausible.
